...
export function presentPackageHeader(data: PackageData): PackageHeaderViewModel {
  ...
  return {
    ...
    stats: buildStats(data, depCount, peerCount),
    hasBundle: data.bundle !== null,
...
function buildStats(data: PackageData, depCount: number, peerCount: number): StatViewModel[] {
  const stats: StatViewModel[] = []

  if (data.bundle) {
    stats.push({ label: 'Bundle Size', value: formatBytes(data.bundle.size), sub: formatBytes(data.bundle.gzip) + ' gzipped', valueColor: null })
  }
